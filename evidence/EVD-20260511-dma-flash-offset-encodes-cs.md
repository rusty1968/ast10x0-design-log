...
A `cs` argument on `dma_read` would be **strictly redundant** given a valid
`flash_offset`. Any caller that constructs a `flash_offset` in `[0, capacity_bytes)`
has already implicitly selected a CS — the offset arithmetic determines which device
the hardware will address regardless of what a `cs` argument says. Accepting both and
checking consistency would be defensive but adds complexity; accepting only
`flash_offset` and deriving the CS internally is simpler, equally safe, and matches
how the underlying offset arithmetic works.
